...
export const getAIProductInsight = async (productName: string, category: string) => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: `بصفتك خبير تجميل طبي متخصص، قدم تحليلاً طبياً موجزاً (30 كلمة كحد أقصى) لمنتج اسمه "${productName}" في فئة "${category}". ركز على الفوائد الملموسة والجانب العلمي للمنتج بأسلوب راقٍ.`,
    });
